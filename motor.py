class Motor(object):
    ...
    def ctrl_alloc(self, direction, pwm):
        ...
        Input arguments:
        - 'direction' : "fwd" or "bck" indicating direction of motor
        ...
        Example usage:
        motor_left.ctrl_alloc("fwd", 75)
        # This sets the left motor direction to forward and 75% duty cycle

        """

        # Check that 'direction' is a valid direction and that PWM is a valid value
        valid_parameters = True
        if direction != "fwd" and direction != "bck":
            print("First parameter (direction) must be either 'fwd' or 'bck'.")
            valid_parameters = False

        if pwm < 0 or pwm > 100:
            print("First parameter (PWM) must lie within the range [0,100].")
            valid_parameters = False
